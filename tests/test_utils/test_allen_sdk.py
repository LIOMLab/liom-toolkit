...
# ---------------------------------------------------------------------------
# Task 3 — 25µm regression test against committed allensdk snapshot fixture
# ---------------------------------------------------------------------------
...
def test_export_itksnap_labels_25um_matches_allensdk_fixture():
    """Byte-exact reproduction of allensdk export_itksnap_labels on 25µm.

    Reads the committed allensdk snapshot fixture (labels.parquet +
    annotation_25um.npz) and compares against the rewritten
    ``construct_reference_space(...).export_itksnap_labels()`` output, which
    reads the cached ``annotation_25.nrrd`` + ``structure_tree.json`` from the
    same fixture directory (no network, no allensdk).

    The fixture is generated by ``regenerate.py`` (a gated one-time dev script
    run on Python 3.12 with allensdk installed). If the fixture files don't
    exist yet, the test skips with a message directing to regenerate.py.
    """
    fixture_dir = _FIXTURE_DIR
    parquet_path = os.path.join(fixture_dir, "labels.parquet")
    npz_path = os.path.join(fixture_dir, "annotation_25um.npz")
    nrrd_path = os.path.join(fixture_dir, "annotation_25.nrrd")
    ...
    if not (os.path.exists(nrrd_path) and os.path.exists(tree_path)):
        pytest.skip(
            "Cached annotation_25.nrrd / structure_tree.json not found in fixture dir. "
    ...
    # looks for "allen_atlas_25.nrrd" — symlink/copy is handled by the test
    ...
    with tempfile.TemporaryDirectory() as data_dir:
        ...
        shutil.copy(nrrd_path, os.path.join(data_dir, "allen_atlas_25.nrrd"))
        shutil.copy(tree_path, os.path.join(data_dir, "structure_tree.json"))
        rs = construct_reference_space(data_dir, resolution=25)
