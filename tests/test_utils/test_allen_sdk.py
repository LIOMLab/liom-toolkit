...
patch ``requests.get`` so no real HTTP call is made. The 100um regression test
...
def _sample_flat_structure_tree() -> list[dict]:
    ...
    committed 100µm regression fixture (``structure_tree.json``) uses this
...
class TestFlattenStructureTreeFlatFormat:
    ...
    ``children`` key, no ``color_hex_triplet``. The committed 100µm regression
...
# ---------------------------------------------------------------------------
# Task 3 — 100µm regression test against committed fixture
# ---------------------------------------------------------------------------
...
def test_export_itksnap_labels_100um_matches_fixture():
    """Regression test for ``export_itksnap_labels`` on the 100µm atlas.

    Reads the committed fixture (``labels.parquet`` + ``annotation_100um.npz``
    + cached ``annotation_100.nrrd`` + ``structure_tree.json``) and compares
    against the rewritten
    ``construct_reference_space(...).export_itksnap_labels()`` output, which
    reads the cached files from the fixture directory (no network, no
    allensdk).

    The 100µm atlas is used instead of the 25µm atlas for speed: the 25µm
    annotation (528x320x456 uint32, 77M voxels) made the ``np.unique`` remap
    take ~7.4s; the 100µm annotation (132x80x114, ~1.2M voxels) runs the same
    remap in ~0.04s. The remap logic (``_remap_to_id_type``) is
    resolution-independent, so 100µm exercises the same code path.

    Oracles:
      * ``labels.parquet`` — the byte-exact allensdk 2.16.2 DataFrame oracle
        (resolution-independent: the 8-column ITK-SNAP metadata is derived
        from the structure tree, not the annotation volume). This remains a
        true independent comparison against the original allensdk output.
      * ``annotation_100um.npz`` — a snapshot of the library's own remapped
        100µm volume. allensdk does not install on Python 3.12+, so the volume
        oracle is a regression guard against unintended changes to the remap
        logic, not an independent allensdk comparison. The DataFrame check
        carries the independent-correctness guarantee.

    The fixture is generated by ``regenerate.py`` (a gated one-time dev
    script). If the fixture files don't exist yet, the test skips with a
    message directing to regenerate.py.
    """
    fixture_dir = _FIXTURE_DIR
    parquet_path = os.path.join(fixture_dir, "labels.parquet")
    npz_path = os.path.join(fixture_dir, "annotation_100um.npz")
    nrrd_path = os.path.join(fixture_dir, "annotation_100.nrrd")
    tree_path = os.path.join(fixture_dir, "structure_tree.json")

    if not (os.path.exists(parquet_path) and os.path.exists(npz_path)):
        pytest.skip(
            "100µm fixture not generated yet. See "
            "tests/test_utils/fixtures/allen_itksnap_100um/regenerate.py for instructions."
        )

    if not (os.path.exists(nrrd_path) and os.path.exists(tree_path)):
        pytest.skip(
            "Cached annotation_100.nrrd / structure_tree.json not found in fixture dir. "
            "Re-run regenerate.py to produce them."
        )

    expected_df = pd.read_parquet(parquet_path)
    expected_vol = np.load(npz_path)["arr"]

    # Point construct_reference_space at a temp data_dir that has the cached
    # files under the names it expects (allen_atlas_100.nrrd +
    # structure_tree.json) — no network, no allensdk.
    import shutil
    import tempfile
    ...
    with tempfile.TemporaryDirectory() as data_dir:
        shutil.copy(nrrd_path, os.path.join(data_dir, "allen_atlas_100.nrrd"))
        shutil.copy(tree_path, os.path.join(data_dir, "structure_tree.json"))
        rs = construct_reference_space(data_dir, resolution=100)

    actual_vol, actual_df = rs.export_itksnap_labels()

    pd.testing.assert_frame_equal(actual_df, expected_df)
    np.testing.assert_array_equal(actual_vol, expected_vol)
